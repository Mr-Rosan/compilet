...
bool LinearScan::linearScanRegisterAllocation()
{
    ...
    for (int i = 4; i < 11; i++)
        regs.push_back(i);
    for (auto& i : intervals)//对于每一个活跃区间。
    {
        expireOldIntervals(i);
        if (regs.empty())// 如果寄存器已经分配完毕
        {
            spillAtInterval(i);// spill register
            success = false;
        }
        else
        {
             // 将一个未被分配的寄存器分配给当前变量i, 并将i加入到active集合中
            i->rreg = regs.front();
...
void LinearScan::expireOldIntervals(Interval* interval)// 处理已经处理过的变量。
{
    auto it = active.begin();
    while (it != active.end())
    {
        //如果变量J的生命周期在i开始前结束，那么将J的变量加入到未分配的寄存器池中。
        if ((*it)->end >= interval->start)
            return;
        regs.push_back((*it)->rreg);//将it的寄存器加入可分配寄存器。
        it = active.erase(find(active.begin(), active.end(), *it));
        sort(regs.begin(), regs.end());
    }
}

void LinearScan::spillAtInterval(Interval* interval)//从active取最后一个interval。
{
    auto spill = active.back();
    if (spill->end > interval->end)// 如果spill结束位置比i还大
    {
        spill->spill = true;
        interval->rreg = spill->rreg;// spill的寄存器分配给i 
