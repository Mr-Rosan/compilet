...
void MachineOperand::output()
{
    ...
    case LABEL:
        ...
        else if (this->label.substr(0, 1) == "@")//全局
...
void MachineBlock::output()
{
    ...
    if (!inst_list.empty())
    {
        ...
        for (auto it = inst_list.begin(); it != inst_list.end(); it++)
        {
            ...
            if (param_num > 4 && (*it)->isStore())// 12/5 函数参数大于四个，则由store指令时需要在栈中参数的上面.
            {
                ...
                if (operand->isReg() && operand->getReg() == 3)
                {
                    ...
                    else
                    {
                        ...
                        offset += 4;
                        auto cur_inst = new LoadMInstruction(this, r3, fp, off);// 12/5 从栈中把后续参数取出。
                        cur_inst->output();
                        //fprintf(stdout,"############\n");
                    }
                }
            ...
            //sp
            if((*it)->isAddInst() && (*it)->getDef()[0]->isReg() && (*it)->getDef()[0]->getReg() == 13 && (*it)->getUse()[0]->isReg() && (*it)->getUse()[0]->getReg() == 13 && (*(it + 1))->isBXInst())
            {
                //fprintf(stdout,"################\n");
                int trueSize = parent->getAllocSpace();
                if (!islegel(trueSize))// invalid constant.数字太大，找寄存器存。
                {
                    //fprintf(stdout,"###############\n");
                    auto r1 = new MachineOperand(MachineOperand::REG, 1);
                    auto off = new MachineOperand(MachineOperand::IMM, trueSize);
                    (new LoadMInstruction(nullptr, r1, off))->output();
                    (*it)->getUse()[1]->setReg(1);
                }
                else
                    (*it)->getUse()[1]->setVal(trueSize);
            }
            (*it)->output();
            count++;
            //fprintf(stdout,"#############\n");
            if (count == MAXINST)//LTORG声明一个数据缓冲池。
            {
                count=0;
                fprintf(yyout, "\tb .B%d\n", label);
                fprintf(yyout, ".LTORG\n");
...
void MachineFunction::output()
{
    ...
    if(islegel(off))
    {
        auto bigSubSp = new BinaryMInstruction(nullptr, BinaryMInstruction::SUB, sp, sp, size);
        bigSubSp->output();
    }
    else
    {
        auto r4 = new MachineOperand(MachineOperand::REG, 4);
        auto loadInst = new LoadMInstruction(nullptr, r4, size);
        loadInst->output();
        auto subSp = new BinaryMInstruction(nullptr, BinaryMInstruction::SUB, sp, sp, r4);
        subSp->output();
    }
    ...
    for (auto iter : block_list)
    {
        ...
        if(count > MAXSIZE)
        {
            fprintf(yyout, "\tb .F%d\n", parent->getGlobalNum());
            fprintf(yyout, ".LTORG\n");
            for (auto s : parent->getGlobals())
            {
                IdentifierSymbolEntry* se = (IdentifierSymbolEntry*)s;
                fprintf(yyout, "addr_%s%d:\n", se->toStr().c_str(), parent->getGlobalNum());
                fprintf(yyout, "\t.word %s\n", se->toStr().c_str());
            }
            parent->addGlobalNum();
...
void MachineUnit::PrintGlobalDecl()
{
    ...
    for (long unsigned int i = 0; i < global_list.size(); i++)
    {
        ...
        else
        {
            ...
            {
                int n = se->getType()->getSize() / INTSIZE;
                int* v = se->getArrayValue();
                for (int i = 0; i < n; i++)
                    fprintf(yyout, "\t.word %d\n", v[i]);
            }
        }
